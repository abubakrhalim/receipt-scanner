...
**Model:** `claude-sonnet-4-6` (Claude claude-sonnet-4-6 — excellent at document/image understanding)
...
The API key is typed into the browser and sent directly to `api.anthropic.com`. It is **never logged or stored** by this app. For production use, proxy the request through a backend to keep the key server-side.
